d3.csv('./senate_committee_data.csv', function(d, i, headers) {
  ...
  function makeLinks(nodes){
    var links = [];
    for(var i = 0; i< nodes.length; i++){ //for each senator, check match with another senator
      for(var j = i+ 1; j< nodes.length; j++){
        var s1 = nodes[i];
        var s2 = nodes[j];
        for( var k = 0; k < s1.committees.length; k++){ //loop over all committees in first node
          var committee = s1.committees[k];
          if(s2 && s2.committees.includes(committee)){
            links.push({
              source: s1.name,
              target: s2.name
            });
